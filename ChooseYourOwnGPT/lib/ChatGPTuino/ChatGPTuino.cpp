...
JsonDocument ChatGPTuino::generateJsonRequestBody()
{
  ...
  doc["model"] = _model;
  // Use max_completion_tokens (replaces the deprecated max_tokens parameter).
  // Newer OpenAI models (o1, o3, gpt-5.x, etc.) only accept this field name.
...
bool ChatGPTuino::putResponseInMsgArray(SecureClient *pClient)
{
  ...
  JsonDocument filter;
  // Include the error field so we can log API-level errors (e.g. unsupported
  // parameter, rate limit, invalid key) alongside the normal response path.
  ...
  // Guard against an API-level error response (e.g. unsupported parameter,
  // invalid key, rate limit). In those cases the response has no "choices"
  // field, so "content" is null. Accessing strlen() on a null pointer causes
  // a LoadProhibited hard fault on ESP32.
  const char *newMsg = jsonResponse["choices"][0]["message"]["content"];
  if (!newMsg)
  {
    const char *apiErr = jsonResponse["error"]["message"];
    if (apiErr)
    {
      Serial.printf("    | putResponseInMsgArray | API error: %s\n", apiErr);
    }
    else
    {
      Serial.println("    | putResponseInMsgArray | no content in response");
    }
    return 0;
  }
  ...
  Serial.print("    | putResponseInMsgArray | strlen ");
  Serial.println(strlen(newMsg));
#endif

  putMessage(newMsg, strlen(newMsg), Assistant);
